...
1. <br/>$$
\begin{array}{c|lcr}
  & \text{8} & \text{16} & \text{32} & \text{64} & \text{平台相关}\\
\hline
...
1. <br/>$$
\begin{array}{c|lcr}
  & \text{8} & \text{16} & \text{32} & \text{64} & \text{128} & \text{平台相关}\\
\hline
...
#### 2. compound data type
1. [T;	N] :	A	fixed-size	array,	for	the element type, T,and	the	non-negative
compile-time	constant	size	N.
2. [T] :	A	dynamically-sized	view	into	a	contiguous	sequence,	for	any	type	 T
3. (T,	U,	..) :	A	finite	sequence,	(T,	U,	..)	where	T	and	U	can	be	different	types.
4. fn(i32)	->	i32 :	A	function	that	takes	an	 i32 	and	returns	an	 i32 .	Functions	also
have	a	type.
